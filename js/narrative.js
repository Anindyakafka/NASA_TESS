...
function drawScene3() {
	...
	function applyFilters() {
		...
		if (tempValue) {
			filteredData = filteredData.filter(planet => planet.pl_eqt >= tempValue)
		}
		if (eccenValue) {
			filteredData = filteredData.filter(planet => planet.pl_orbeccen <= eccenValue)
		}
		if (radiusValue) {
			filteredData = filteredData.filter(planet => planet.pl_radj <= radiusValue)
		}
		if (smassValue) {
			filteredData = filteredData.filter(planet => planet.st_mass <= smassValue)
		}
		if (sradValue) {
			filteredData = filteredData.filter(planet => planet.st_rad <= sradValue)
		}
